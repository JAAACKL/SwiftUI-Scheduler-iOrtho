...
struct EventCellView: CellableView {
    ...
    struct ViewModel: ICEventable {
        ...
        init(text: String, description: String, start: Date, end: Date?, isAllDay: Bool = false, editState: EditState? = nil, color: Color? = Color.google.defaultColor) {
            self.text = text
        ...
        func copy() -> EventCellView.ViewModel {
            var copy = ViewModel(text: text, description: description, start: startDate, end: endDate, isAllDay: isAllDay, editState: editState, color: color)
            copy.id = id
        ...
        static func create(from eventable: EventCellView.ViewModel?, state: EditState?) -> EventCellView.ViewModel {
            ...
            return ViewModel(text: "Patient Appointment", description: "First Visit of New Patient.\nPrepare for Oral Examination.", start: Date(), end: nil, editState: state)
        }
    }
