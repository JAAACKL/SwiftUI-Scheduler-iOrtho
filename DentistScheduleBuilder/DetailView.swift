        ...
        VStack(alignment: .leading) {
            HStack(alignment: .top, spacing: 24.0) {
                ...
                VStack(alignment: .leading, spacing: 16.0) {
                    Image("iOrtho").resizable().aspectRatio(contentMode: .fit).frame(width: 250)
                    Text(item.text).font(.system(size: 60))
                    Text(schedule(dates: item.startDate...(item.endDate ?? Date()), isAllDay: item.isAllDay)).font(.system(size: 40))
                    Text(item.description).lineLimit(5).font(.system(size: 50))
